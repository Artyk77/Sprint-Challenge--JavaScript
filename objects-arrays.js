...
const universities = [];
graduates.forEach(char => universities.push(char.university)); 
...
const contactInfo = [];
graduates.forEach(list => contactInfo.push(`${list.first_name} ${list.email}`)); 
console.log(contactInfo);


/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all.
 Log the result. */

 const uni = [];
universities.filter(word => {
  if(word.includes('Uni')){
    uni.push(word);
  }
});
console.log(uni);
...
*/
const animalNames = [];
zooAnimals.forEach(i => animalNames.push(`${i.animal_name}, ${i.scientific_name}`))
...
const lowerCase = [];
zooAnimals.map(lower => lowerCase.push(lower.animal_name.toLowerCase()));
...
const pops = [];
zooAnimals.forEach(el => pops.push(el.population));
console.log(pops);
const populationTotal = pops.reduce((acu, cur) => acu+=cur);
